...
public class MouseLookScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Re-lock the mouse cursor when pressing L
    ...
    {
        /*
        * Adjusting mouse sensitivity depending on whether we're aiming or not.
        */
        if (Input.GetAxis("Fire2") != 0)
        {  // Right mouse button (Fire2) for aiming
    ...
    public float yRotationSpeed = 0.1f, xCameraSpeed = 0.1f;
    ...
    {
        // Adjust Y and X rotations based on mouse input
        wantedYRotation += Input.GetAxis("Mouse X") * mouseSensitvity;  // Horizontal movement
        wantedCameraXRotation += Input.GetAxis("Mouse Y") * mouseSensitvity;  // Vertical movement

        // Clamp the camera's vertical movement between top and bottom angles
        wantedCameraXRotation = Mathf.Clamp(wantedCameraXRotation, bottomAngleView, topAngleView);
    }

    void ApplyingStuff()
    {
        // Smooth camera movement for more natural feel
        ...
        // Rotate the weapon accordingly
        WeaponRotation();

        // Apply player (horizontal) rotation
    ...
    void FPSCounter()
    {
        ...
        style.alignment = TextAnc  hor.UpperLeft;
