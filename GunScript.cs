...
public class GunScript : MonoBehaviour {
	...
	*/
	void FixedUpdate(){
		 ...
		 */
		//if aiming
		if(Input.GetAxis("Aim") != 0 && !reloading && !meeleAttack){
	...
	void Sprint(){// Running();  so i can find it with CTRL + F
		if (Input.GetAxis ("Vertical") > 0 && Input.GetAxisRaw ("Aim") == 0 && meeleAttack == false && Input.GetAxisRaw ("Fire1") == 0) {
	...
	*/
	...
	void DrawCrosshair(){
		...
		if(Input.GetAxis("Aim") == 0){//if not aiming draw
	...
	*/
	void Animations(){
		...
		if(handsAnimator){
			...
			handsAnimator.SetBool("aiming", Input.GetButton("Aim"));
