...
public class PlayerMovementScript : MonoBehaviour {
	...
	// Getting the Players rigidbody component and grabbing the mainCamera from Players child transform.
	
    void Awake(){
        rb = GetComponent<Rigidbody>();

        // Find the camera and check if it exists
        ...
        if (cameraMain != null)
        {
            ...
            if (bulletSpawn == null)
            {
                Debug.LogError("BulletSpawn not found under Main Camera.");
            }
        }
        else
        {
	...
	*/
	void PlayerMovementLogic(){
        ...
        if (grounded)
        {
            rb.AddRelativeForce(new Vector3(Input.GetAxis("Horizontal") * accelerationSpeed, 0, Input.GetAxis("Vertical") * accelerationSpeed), ForceMode.Acceleration);
        }
        else
        {
            rb.AddRelativeForce(new Vector3(Input.GetAxis("Horizontal") * accelerationSpeed, 0, Input.GetAxis("Vertical") * accelerationSpeed), ForceMode.Acceleration);
	...
	*/
	...
	public float accelerationSpeed = 5000.0f;
	...
	*/
    ...
    [Tooltip("Put BulletSpawn gameobject here, place from where bullets are created.")]
    [HideInInspector]
